...
package in `PACKAGE_BASE` (`@heroiclands/package-build/engine/kb-manifest`), prefixed when the address is
resolved:
...
rather than editing it in place. `READABLE_VERSIONS` in `@heroiclands/package-build/engine/kb-manifest`
...
`@heroiclands/package-build/engine/kb-manifest` lists the packages that exchange manifests
