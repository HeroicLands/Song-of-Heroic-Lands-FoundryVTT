...
See also: [[doc-architecture|Architecture Overview]], [[doc-houserulescookbook|House Rules Cookbook]], [[doc-securitymodel|Security Model]].
...
- Use **actions** (context-menu entries on a document) for single-item behavior overrides (e.g., one specific spell). A Script Action can also **override an intrinsic action** on that one document by reusing its `shortcode` — see [[doc-macrosandactions|Overriding an intrinsic action]].
...
When a hook performs persistent side effects, guard it with a world-setting toggle **and** a GM-only check, so the rule is opt-in per world and runs under a single authority. See the worked recipe in [[doc-houserulescookbook#recipe-3-the-recommended-guard-pattern|House Rules Cookbook — guard pattern]].

## Golden rule

**Extend by adding new classes and registering them**, rather than editing core logic in-place — see the [[doc-architecture#architectural-rules|architectural rules]] and [[doc-architecture#extension-mechanisms|extension mechanisms]].
...
The per-type split into Document, DataModel, Logic, and Sheet classes — and how they relate — is covered by the [[doc-architecture#three-layer-architecture|three-layer architecture]] and [[doc-architecture#three-class-pattern|three-class pattern]]; read those rather than re-deriving them here.
...
This is the canonical "add a type" procedure — the [[doc-runtimecontracts|Runtime Contracts]] safe-extension checklist points here.
...
## 3) Combat / tests / resolution pipeline {#3-combat-tests-resolution-pipeline}
...
### Adding a graded / special-result test — pass data, don't subclass {#adding-a-graded-special-result-test-pass-data-dont-subclass}
...
- **`scope.resultDescTable`** — a
  [[doc-resultdescriptiontables|result-description table]] (`LimitedDescription[]`) that
...
- **`scope.priorTestResult`** _(optional)_ — reuse an already-rolled result instead
  ...
  see the [[doc-modifiermodel|prior-result seam]].
...
[[doc-resultdescriptiontables#the-tochat-card-data-contract|`toChat` card-data contract]].
...
See [[doc-combatresolutionpipeline|Combat Resolution Pipeline]] and [[doc-modifiermodel|Modifier Model]].

## 4) Active effects

Core: [SohlActiveEffect](../../../../src/document/effect/foundry/SohlActiveEffect.ts).

SoHL extends Foundry's ActiveEffect with an expanded targeting model (`targetType` / `targetName`) so one effect can target self, the owning actor, or sibling items by type. See [[doc-architecture#active-effects|Active Effects]] for the model and [[doc-effectsintegration|Effects Integration]] for the full reference.
...
### Adding a chat-card button {#adding-a-chat-card-button}
...
[[doc-runtimecontracts#chat-card-dispatch-contract|Chat-card dispatch contract]].
...
See [[doc-runtimecontracts#passing-scope-to-a-button|Chat-card dispatch contract — Passing scope]]
and the [[doc-runtimecontracts#entity-serialization-contract|Entity serialization contract]].

### Cross-actor effects (the acknowledge-button pattern) {#cross-actor-effects-the-acknowledge-button-pattern}

Enforce **[[doc-architecture#actor-state-sovereignty|actor state sovereignty]]**: an actor mutates only itself. To make one actor affect another, **never reach into the target** — instead:
...
[[doc-shortcodeintegrity|Shortcode Integrity]]. The Create dialog
...
- [[doc-macrosandactions|Macros and Actions]]
- [[doc-lifecyclehooks|Lifecycle Hooks]]
- {@link sohl.core.logic.SohlLogic}
- [[doc-combatresolutionpipeline|Combat Resolution Pipeline]]
- [[doc-modifiermodel|Modifier Model]]
- [[doc-effectsintegration|Effects Integration]]
- [[doc-runtimecontracts|Runtime Contracts]]
