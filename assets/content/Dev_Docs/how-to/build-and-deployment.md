...
> [[doc-systemdevelopment|System Development]].
...
| `lint:expr-scopes`        | Fail if the generated expression-scope table in [[doc-expressions                                                                                                                                                                              \| Expressions and Scripts]] is out of date with `src/entity/expr/expression-scopes.mjs`. Regenerate with `npm run docs:expr-scopes`. |
...
[[doc-cssarchitecture|CSS Architecture]] tells module authors to build
...
`gitignore: true` keeps the generated trees out (`build/`, `nogit/`), and
`CHANGELOG.md` is excluded by the shared configuration, since
`changeset version` regenerates it in every repository here.
`.markdownlint-cli2.mjs` re-exports that shared configuration and adds
nothing, because nothing about this repository's layout is unknown to it. The
split is the same as Prettier's: `.prettierignore` stays here, the rules do
not.
...
1. **`lint`** — every `lint:*` script in parallel: `lint:format` (Prettier
   reports no drift anywhere in the repo), `lint:markdown` and `lint:styles`
   (see [What the two linters check](#what-the-two-linters-check)), the
   content and generated-page guards, `lint:dts` (the generated public type
...
## 5. Compendium packs from in-repo Markdown {#5-compendium-packs-from-in-repo-markdown}
...
### An item's prose compiles to a journal, not into the item {#an-items-prose-compiles-to-a-journal-not-into-the-item}
...
> [[doc-contentlinks|Linking Between Content Notes]] instead — this
...
## 6. Deploying to a Foundry instance {#6-deploying-to-a-foundry-instance}
...
[[doc-writingchangesets|Writing Changesets]]):
...
| ------------ | ------------------------ | --------------------- | ---------------------- |
| `/sohl/`     | The package landing page | Hugo (`homepage.md`)  | `main`                 |
...
| `build-type-catalog.mjs` | Generate `assets/content/Dev_Docs/reference/type-catalog.md` from the kind enums.                                                          |
...
- [[doc-gettingstarted|Getting Started]] — the codebase tour for a new developer.
- [[doc-systemdevelopment|System Development]] — the rules of
  contributing and the PR workflow.
- [[doc-writingchangesets|Writing Changesets]] — recording a change
  for the changelog and release.
- [[doc-testing|Testing]] — the test tooling and patterns.
