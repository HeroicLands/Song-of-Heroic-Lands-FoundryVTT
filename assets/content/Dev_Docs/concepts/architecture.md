...
See also: [[doc-devdocs|Documentation Hub]], [[doc-extensionpoints|Extension Points]], {@link sohl.core.logic.SohlLogic}.
...
- On **{@link sohl.document.actor.logic.BeingLogic}** directly: `health`, `healingBase` (a `ValueModifier` whose base is the average of the being's Endurance and Will scores — rounded up when END > WIL, else down — and which trait/treatment deltas layer onto; multiplied by a Healing Rate it is the target of nearly every recovery test), `shockState` (an ascending severity level None…Dead derived from the Stunned/Incapacitated/Unconscious/Dead **status effects** — not a stored field; set it through `setShockState` / `advanceShockState`, never by toggling statuses directly), `fatiguePenalty` (a `ValueModifier` summing the Fatigue Levels of every `fatigue`-subtype trauma — windedness/weariness/weakness recorded as separate instances — which penalizes tests and Move rate), `pull`, and `carriedWeight` (the ground-up total of carried gear).
...
## Phase-batched lifecycle {#phase-batched-lifecycle}
...
See {@link sohl.core.logic.SohlLogic} for the full rationale and rules, and [[doc-lifecyclehooks|Lifecycle Hooks]] for augmenting a phase from a module.
...
For the classes in each area, see **Domain** in the API reference, and [[doc-modifiermodel|Modifier Model]], [[doc-combatresolutionpipeline|Combat Resolution Pipeline]], and [[doc-bodystructure|Body Structure]].
...
See [[doc-effectsintegration|Effects Integration]].
...
1. **Modules via lifecycle hooks** — Foundry modules listen for hooks emitted at each lifecycle phase to augment behavior broadly across items or actors. See [[doc-lifecyclehooks|Lifecycle Hooks]].

2. **Actions** — document-attached executable logic surfaced on context menus, for per-item behavior. See [[doc-macrosandactions|Macros and Actions]].

## FoundryHelpers shim {#foundryhelpers-shim}
...
| Extending SoHL        | [[doc-extensionpoints             \| Extension Points]]           |
| Lifecycle hooks       | [[doc-lifecyclehooks              \| Lifecycle Hooks]]            |
| Actions               | [[doc-macrosandactions            \| Macros and Actions]]         |
| Expressions/scripts   | [[doc-expressions                 \| Expressions and Scripts]]    |
| Security & guardrails | [[doc-securitymodel               \| Security Model]]             |
| Combat resolution     | [[doc-combatresolutionpipeline    \| Combat Resolution Pipeline]] |
| Modifier system       | [[doc-modifiermodel               \| Modifier Model]]             |
| Body anatomy          | [[doc-bodystructure               \| Body Structure]]             |
| Active effects        | [[doc-effectsintegration          \| Effects Integration]]        |
| CSS/styling           | [[doc-cssarchitecture             \| CSS Architecture]]           |
| Testing               | [[doc-testing                     \| Testing Guide]]              |
| Type catalog          | [[doc-typecatalog                 \| Type Catalog]]               |
| Runtime contracts     | [[doc-runtimecontracts            \| Runtime Contracts]]          |
