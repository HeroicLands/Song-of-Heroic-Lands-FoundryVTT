...
## The core principle: reference code, never compile it from data {#the-core-principle-reference-code-never-compile-it-from-data}
...
[[doc-runtimecontracts#entity-serialization-contract|Entity serialization contract]];
...
### Why not a sandbox / denylist? {#why-not-a-sandbox-denylist}
...
### GM "homebrew": use Foundry macros, not a bespoke sandbox {#gm-homebrew-use-foundry-macros-not-a-bespoke-sandbox}
...
### Extension points: which tool for which need {#extension-points-which-tool-for-which-need}
...
- **Chat-card clicks are authorized by the handler document's ownership.** A card
  ...
  [[doc-runtimecontracts#authorization-the-handler-documents-owner-acts|Chat-card dispatch contract — Authorization]].
- **Actor-state sovereignty.** An actor mutates only itself. Cross-actor effects
  ...
  own client. See [[doc-architecture|Architecture]] and
