...
both are built on [[doc-sohlapi|The SoHL API]]. They are **different things**:
...
the document surface for a specific thing; see [[doc-sohlapi|The SoHL API]]).
...
[[doc-securitymodel#the-core-principle-reference-code-never-compile-it-from-data|Security Model]].
...
## How SoHL uses this internally: intrinsic actions {#how-sohl-uses-this-internally-intrinsic-actions}
...
Logic class — see [[doc-extensionpoints|Extension Points]] and {@link sohl.entity.action.SohlAction} /
{@link sohl.core.logic.SohlLogic} in the API reference. To affect _all_ documents of a type rather
than attach one action, use [[doc-lifecyclehooks|Lifecycle Hooks]].
...
[[doc-actioncards|Action Cards & the Consent Model]].
...
- [[doc-sohlapi|The SoHL API]] — the document and `sohl` surfaces these scripts use.
- [[doc-lifecyclehooks|Lifecycle Hooks]] — type-wide behavior instead of a
  per-document action.
- [[doc-extensionpoints|Extension Points]] — defining intrinsic actions and new types.
- [[doc-moduledevelopment|Writing Modules]] — when a script outgrows a macro or
  action.
