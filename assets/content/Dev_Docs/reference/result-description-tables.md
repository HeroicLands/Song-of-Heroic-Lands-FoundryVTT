...
> [[doc-securitymodel|security model]]), so a table with a raw function
...
[[doc-extensionpoints#adding-a-graded-special-result-test-pass-data-dont-subclass|Extension Points §3]].
The other half — offering a follow-up action from the result — is the `buttons`
input documented next.

## The `toChat` card-data contract {#the-tochat-card-data-contract}
