...
See also: [[doc-architecture|Architecture Overview]], [[doc-extensionpoints|Extension Points]], [[doc-securitymodel|Security Model]].
...
## The grounding rule: reference on the wire, live object in memory {#the-grounding-rule-reference-on-the-wire-live-object-in-memory}
...
## Entity serialization contract {#entity-serialization-contract}
...
## Chat-card dispatch contract {#chat-card-dispatch-contract}
...
The resolved document's `onChatCardButton(btn)` (or `onChatCardEditAction`) is then invoked; handlers switch on `btn.dataset.action`. New card buttons must emit one of the attributes above and add an `action` case rather than introducing a new attribute name. For the procedure, see [[doc-extensionpoints#adding-a-chat-card-button|Extension Points — Adding a chat-card button]].

### Authorization: the handler document's owner acts {#authorization-the-handler-documents-owner-acts}

**A chat-card action is _handled by the actor it addresses_, and only a client that owns that actor may run it** (a GM owns all). Buttons that drive a flow — roll damage, respond to an attack, resume an opposed test — mutate the handler's own actor state, so under [[doc-architecture#actor-state-sovereignty|actor-state sovereignty]] authorization is exactly document ownership. This rule has **two sides**, both keyed on the resolved handler document's `isOwner`:
...
### Passing `scope` to a button {#passing-scope-to-a-button}
