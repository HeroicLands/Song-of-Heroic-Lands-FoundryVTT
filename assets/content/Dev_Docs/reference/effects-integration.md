...
See also: [[doc-modifiermodel|Modifier Model]], [[doc-extensionpoints|Extension Points]], [[doc-securitymodel|Security Model]].
...
SoHL ships its own dispatcher for document-level subscriptions ([[doc-eventqueue|`sohl.events`]]). The trigger names and context shapes are **identical to Foundry's `CONFIG.ActiveEffect.expiryEvents` vocabulary** so that an Active Effect with `duration.expiry: "turnStart"` and a SoHL subscription on `"turnStart"` are responding to the same moment, in the same shape, with the same data.
...
See [[doc-eventqueue|Event Queue]] for the subscription model, cascading dispatch on `updateWorldTime`, and loop protection.
