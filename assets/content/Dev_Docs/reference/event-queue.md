...
1. **Write the behavior as an action** on the document's logic — a normal
   ...
   `context.scope` (see the [[doc-actioncards#self-sufficient-actions|self-sufficient action contract]]).
...
Under the [[doc-devdocs|Prime Directive]] the queue
...
`curWorldTime()` / `curCombatTime()` [[doc-expressions#the-standard-helpers|expression helpers]].
...
### 8. A scene-region trigger — offer a check on entering {#8-a-scene-region-trigger-offer-a-check-on-entering}
