...
- **First steps:** fork the repo, branch from `main`, run `npm ci`, and copy
  ...
  [[doc-gettingstarted|Getting Started]] for the full setup and codebase
  tour.
- **Learn the mental model before changing anything:**
  [[doc-architecture|Architecture Overview]] — the three-layer design and
  a map of the `src/` tree.

## Core rules (non-negotiable) {#core-rules-non-negotiable}
...
6. **Stable localization keys.** Never rename existing keys in `lang/en.json`; add
   ...
   [[doc-localizationkeys|Localization Keys]] — it is the rule the
...
9. **Never compile data into code.** No `eval`, `new Function`, or
   ...
   Read [[doc-securitymodel|Security Model & Guardrails]] before
...
— see [[doc-issuereporting#9-which-repository-does-an-issue-belong-in|Issue Reporting §9]]
...
- Run `npm run test` before `npm run build`. See [[doc-testing|Testing]].
...
- **User guide journals** (`assets/packs/journals/_source/`) when a player-facing
  workflow changed.
- **Extension points** — [[doc-extensionpoints|Extension Points]] when you added or
  changed one.
...
the issue. See [[doc-writingchangesets|Writing Changesets]] for details.
...
- **JSDoc.** Public-API JSDoc feeds TypeDoc generation — keep it complete and
  ...
  APIs, [[doc-extensionpoints|Extension Points]] for extension points, and
...
- **JSDoc → doc-page links.** The reverse direction — a **JSDoc comment pointing
  ...
  API site is strictly generated symbols; the prose lives on the knowledgebase,
...
- **Stylesheets and markdown are linted, not just formatted.** Prettier owns the
  ...
  [[doc-cssarchitecture|CSS Architecture]] publishes as an extension
...
- **Localization keys.** Every user-visible string is a key in `lang/en.json`, named
  by the standard in [[doc-localizationkeys|Localization Keys]]:
...
- **Foundry v14.** Target Foundry VTT v14+ and follow the v14 patterns described in
  the [[doc-architecture|Architecture Overview]].
...
- [[doc-moduledevelopment|Writing Modules]] — build a Foundry module that integrates with or
  extends SoHL via hooks, registries, the `sohl` public object, and CSS variables.
- [[doc-macrosandactions|Macros and Actions]] — write macro-bar scripts or
  document-attached Script Actions against the SoHL API.
- [[doc-sohlapi|The SoHL API]] — the document and `sohl` surfaces scripts
  and modules use.

These build on the deeper how-to references —
[[doc-extensionpoints|Extension Points]] and
