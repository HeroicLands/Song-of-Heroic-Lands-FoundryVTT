...
/**
 * Register hooks that enhance the Foundry **combatant config sheet** with SoHL's
 * combat-scoped fields: the situational `moveFactor` multiplier and the
 * `displayedMedium` whose computed move the tracker shows.
 *
 * Uses DOM-injection rather than a custom sheet class so the system keeps
