...
export function moveAndSort(items, source, target, before = true) {
    if (!items || !Array.isArray(items) || items.length === 0 || items.some((i) => !(i instanceof Item))) {
    ...
    const siblings = parent ? items.filter((i) => i.nestedIn?.id === parent.id && i.id !== source.id) : [];
    ...
    if (before) {
        insertIndex = target ? work.findIndex((e) => e.target.id === target.id) : work.length;
    } else {
        insertIndex = target ? work.findIndex((e) => e.target.id === target.id) + 1 : work.length;
    }
    const insertIndex =
