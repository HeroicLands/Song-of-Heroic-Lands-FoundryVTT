 ...
 * @enum {string}
 */
export const TemporalDirection = Object.freeze({
    PAST: "past",
    FUTURE: "future",
    NOW: "now",
});
 ...
 * @property {TemporalDirection} [direction]
...
export class SohlLocalize {
    ...
    /**
     * Construct a new SohlLocalize instance.
     * @param {object} options
     * @param {string} options.lang - The language code to use for localization.
     */
    constructor(options = {}) {
        const lang = options.lang ?? game.i18n.lang ?? "en";
        this._lang = lang;
    }
    ...
    sortObjects(objects, key) {
        objects.sort((a, b) => {
            return this.compare(
                sohl.utils.getPathValue(a, key),
                sohl.utils.getPathValue(b, key),
            );
        });
        return objects;
    }
     ...
     */
    static _formatDurationEn(value) {
     ...
     */
    static _formatDurationFi(value) {
        if (value?.direction !== TemporalDirection.NOW) {
            const parts = [];
     ...
     */
    static _formatDurationDe(value) {
        if (value?.direction !== TemporalDirection.NOW) {
            const parts = [];
     ...
     */
    static _formatDurationSv(value) {
        if (value?.direction !== TemporalDirection.NOW) {
            const parts = [];
     ...
     * @returns {string} Formatted string like "2år 3mån 4v"
     */
    static _formatDurationFr(value) {
        if (value?.direction !== TemporalDirection.NOW) {
            const parts = [];
     ...
     */
    static _formatDurationEs(value) {
        if (value?.direction !== TemporalDirection.NOW) {
            const parts = [];
            if (value.years) parts.push(`${value.years}a`);
            if (value.months) parts.push(`${value.months}m`);
    ...
    format(messageKey, values = {}) {
        const { count, ordinal, useFallback } = values;
        ...
        const fmt = /{[^}]+/g;
        str = str.replace(fmt, (k) => values[k.slice(1, -1)]);
        return str || "";
    }
     ...
     */
    localize(stringId, useFallback = false) {
        if (!useFallback) {
