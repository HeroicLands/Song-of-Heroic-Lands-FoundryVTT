/*
 * This file is part of the Song of Heroic Lands (SoHL) system for Foundry VTT.
 * Copyright (c) 2024-2025 Tom Rodriguez ("Toasty") — <toasty@heroiclands.com>
 *
 * This work is licensed under the GNU General Public License v3.0 (GPLv3).
 * You may copy, modify, and distribute it under the terms of that license.
 *
 * For full terms, see the LICENSE.md file in the project root or visit:
 * https://www.gnu.org/licenses/gpl-3.0.html
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

import { SohlItemProxy } from "@logic/common/item";
import { ProtectionData } from "@logic/common/item/data";
import { SohlVariant } from "@logic/common/core";
import { ProtectionPerformer } from "@logic/common/item/performer";
import { SubTypeDataModel } from "./SubTypeDataModel";
const { SchemaField, NumberField } = (foundry.utils as any).fields;

export class ProtectionDataModel
    extends SubTypeDataModel<ProtectionPerformer, SohlVariant>
    implements ProtectionData
{
    protected static readonly logicClass = ProtectionPerformer;
    declare readonly parent: SohlItemProxy<ProtectionPerformer>;
    protectionBase!: {
        blunt: number;
        edged: number;
        piercing: number;
        fire: number;
    };

    static defineSchema() {
        return {
            ...super.defineSchema(),
            protectionBase: new SchemaField({
                blunt: new NumberField({
                    integer: true,
                    initial: 0,
                    min: 0,
                }),
                edged: new NumberField({
                    integer: true,
                    initial: 0,
                    min: 0,
                }),
                piercing: new NumberField({
                    integer: true,
                    initial: 0,
                    min: 0,
                }),
                fire: new NumberField({
                    integer: true,
                    initial: 0,
                    min: 0,
                }),
            }),
        };
    }
}
