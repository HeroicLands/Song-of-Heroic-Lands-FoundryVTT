...
/**
 * Generates `assets/content/Dev_Docs/reference/type-catalog.md` — a `doc` note
 * that compiles into no Foundry document — from authoritative sources, so the
 ...
 * @param {string} dir
 * @returns {Record<string, {file: string, className: string}>}
 */
...
/**
 * Build the type-catalog markdown from the authoritative sources (constants,
 * en.json, and each Logic class's TSDoc). Pure — reads files and returns the
 * markdown and any warnings, without writing. `docs:catalog` writes the result;
 * `lint:type-catalog` compares it against the committed file.
 * @returns {{ md: string, warnings: string[], actorCount: number, itemCount: number }}
 */
export function buildTypeCatalog() {
    ...
    // The note envelope: the page is content, addressed `doc-typecatalog`, and
    // compiles into no Foundry document.
    const md = `---
type: doc
subType: reference
name:
...
`;
    ...
    return {
        md,
        warnings,
        actorCount: actorKinds.length,
        itemCount: itemKinds.length,
    };
}
