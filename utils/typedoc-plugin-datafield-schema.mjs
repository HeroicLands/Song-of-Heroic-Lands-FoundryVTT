...
export function load(app) {
    ...
    app.converter.on(Converter.EVENT_CREATE_DECLARATION, (ctx, reflection, node) => {
        if (reflection.kind === ReflectionKind.Property && node?.decorators) {
            ...
            if (dataField) {
                ...
                const getProp = (name) =>
                    objLiteral.properties.find((p) => p.name?.getText() === name)?.initializer;

                const dataName = stripQuotes(getProp("dataName")?.getText()) || reflection.name;
                const type = stripQuotes(getProp("type")?.getText()) || "unknown";
                const required = getProp("required")?.getText() === "true" ? "yes" : "no";
