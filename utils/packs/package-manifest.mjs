 ...
 *
 * {@link FOUNDRY_PACKAGE_ID} is *declared* in `content-package.mjs` rather than
 * read from the manifest, so the link resolver stays filesystem-free and
...
/** Where the manifest template lives, relative to the repository root. */
export const DEFAULT_TEMPLATE_DIR = "./assets/templates";
 ...
 *
 * @param {string} configuredId - `FOUNDRY_PACKAGE_ID` from `content-package.mjs`.
...
export function assertPackageIdMatchesManifest(
    ...
    if (!configuredId) {
        throw new Error(
            "FOUNDRY_PACKAGE_ID is empty in utils/packs/content-package.mjs. " +
    ...
    if (configuredId !== manifestId) {
        throw new Error(
            `Foundry package id drift: FOUNDRY_PACKAGE_ID is "${configuredId}" ` +
                `in utils/packs/content-package.mjs, but ${manifestPath} ` +
                `declares "${manifestId}". Every compendium UUID takes its first ` +
                "segment from FOUNDRY_PACKAGE_ID, so compiling now would emit " +
                "documents addressing a package this repository does not ship. " +
 ...
 *
 * A repository with no manifest template at all is an error rather than a skip:
 * it compiles packs for a Foundry package, and without that package's manifest
 * there is nothing the emitted UUIDs can be checked against — silently
 * compiling is the very failure this guard exists to prevent.
 *
    ...
    if (!manifestPath) {
        throw new Error(
            `No Foundry package manifest template found in ${dir} — looked for ` +
                ...
                "every document by the shipped package's id, which cannot be " +
                "verified without its manifest.",
        );
    }
