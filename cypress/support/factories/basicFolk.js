...
export const BASIC_FOLK = Object.freeze({
    pack: "sohl.actors",
    // Resolved at runtime by stable `shortcode`/`name` (see `importActor` /
    // `resolveDocId`), never a hardcoded pack `_id` — compendium ids are
    // regenerated by the pack build, so a literal id rots on every rebuild.
    shortcode: "basicfolk",
    name: "Basic Folk",
});
